...
from keyboards.default.ordering_kb import number,payments
...
@dp.message_handler(text="⬅️ Ortga", state=UserState.ordering)
async def go_back(xabar: Message, state: FSMContext):
    data = await state.get_data()
    category = data.get('category', 'Lavash')
    await xabar.answer(text="Quyidagilardan birini tanlang", 
...
@dp.message_handler(text="⬅️ Ortga", state=UserState.payment)
async def go_back(xabar: Message, state: FSMContext):
    data = await state.get_data()
...
async def cancel_ordered_product_handler(call: CallbackQuery, state: FSMContext):
    # product_name = call.data.split('.')
    tg_id = call.from_user.id
    db.delete_all_orders(tg_id,)
  
    await state.update_data(
        {"cart_exists": False}
    )
    await call.answer("Savatchangiz tozalandi")
    await call.message.answer("Bo'limning tanlang", reply_markup=make_categories_kb())
...
async def buyurtma_berish(call: CallbackQuery, state:FSMContext):
    await call.message.answer(text="""Telefon raqamingizni quyidagi formatda yuboring yoki kiriting: +998 ** *** ** **
                           Eslatma: Agar siz onlayn buyurtma uchun Click yoki Payme orqali toʻlashni rejalashtirmoqchi boʻlsangiz, tegishli xizmatda hisob qaydnomasi roʻyxatdan oʻtgan telefon raqamini koʻrsating.""",reply_markup= number)
    await UserState.ordering.set()
